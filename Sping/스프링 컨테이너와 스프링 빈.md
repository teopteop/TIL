...
- 스프링 컨테이너 `ApplicationContext`는 인터페이스이다.
...
> 참고: 더 정확히는 스프링 컨테이너를 부를 때 BeanFactory, ApplicationContext 로 구분해서 이야기 하지만
...
- 애플리케이션 빈 출력하기
  ...
  - 스프링이 내부에서 사용하는 빈은 getRole() 로 구분이 가능하다.
...
#### 예제코드 - ApplicationContextBasicFindTest
```java
package com.example.demo.beanfind;
...
- 그래서 모든 자바 객체의 최고 부모인 **Object 타입으로 조회**하면, **모든 스프링 빈을 조회**한다.
...
- `ApplicationContext`는 빈 관리기능 및 편리한 부가 기능을 제공한다.
...
### XML 설정 사용
- 최근에는 스프링 부트를 많이 사용하면서 XML 기반의 설정은 잘 사용하지 않는다. 하지만 아직 많은 레거시 프로젝트
들이 XML로 되어 있고, 또 XML을 사용하면 컴파일 없이 빈 설정 정보를 변경할 수 있는 장점도 있으므로 한번쯤
배워두는 것도 괜찮다.
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <bean id="memberRepository"
          class="com.example.demo.member.MemoryMemberRepository" />
...
거의 비슷하다는 것을 알 수 있다.
- xml 기반으로 설정하는 것은 최근에 잘 사용하지 않으므로 이정도로 마무리 하고, 필요하면 스프링 공식 레퍼런스
문서를 확인하자.

## 스프링 빈 설정 메타 정보 - BeanDefinition
- 스프링이 다양한 설정 형식을 지원하는 핵심에는 BeanDefinition 이라는 추상화가 있다.
- **역할과 구현을 개념적으로 나눈 것**
...
### 정리
- `BeanDefinition`을 직접 생성해서 스프링 컨테이너에 등록할 수 도 있다. 하지만 실무에서 `BeanDefinition`을
직접 정의하거나 사용할 일은 거의 없다.
- **`BeanDefinition`에 대해서는 너무 깊이있게 이해하기 보다는, 스프링이 다양한 형태의 설정 정보를
`BeanDefinition`으로 추상화해서 사용하는 것 정도만 이해하면 된다.**
